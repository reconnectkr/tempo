...
- 완료된 테스크는 당일 동안 취소선과 흐린 색상으로 표시
- 자정 지나면 완료된 테스크는 사라짐
...
```
┌─────────────────────────────────────┐
│ Tempo              2026-04-26 (일)  │  ← 헤더 (앱 이름 + 오늘 날짜)
├─────────────────────────────────────┤
│                                     │
│  ○  보고서 작성       play  25:00   │  ← 메인 테스크
│       ○  자료 수집                  │  ← 하위 1단계
│            ○  시장 데이터            │  ← 하위 2단계
│       ●  목차 작성       (완료)     │  ← 완료된 테스크 (취소선)
│                                     │
│  ○  코드 리뷰        pause  14:32   │  ← 타이머 실행 중
│                                     │
├─────────────────────────────────────┤
│  [입력...]                    [↵]   │  ← 항상 고정된 입력창
│                                     │
...
|------|------------------|
| 미완료 체크박스 | `circle` |
| 완료 체크박스 | `checkmark.circle.fill` |
| 타이머 시작 버튼 | `play.fill` |
| 타이머 일시정지 | `pause.fill` |
| 입력 Enter | `return` 또는 `arrow.turn.down.left` |
...
**완료 표시**:
- 텍스트에 취소선 (`.strikethrough()`)
- 텍스트 색상 `Color.secondary` 또는 `Color.tertiary`

**선택된 테스크**:
- 행 배경에 미세한 강조색 (시스템 selection color)
- 입력창 아래에 "→ '{선택된 테스크 제목}'의 하위로 추가" 안내 텍스트

**드롭다운 크기**:
- 너비 360pt 권장 (메뉴바 앱 표준)
- 높이는 컨텐츠에 따라 가변. 테스크가 많으면 스크롤 가능
- 최소 높이 200pt, 최대 높이 600pt 정도
...
### 8.6 macOS 권한
- 알림 권한 (필수)
- 자동시작은 `ServiceManagement` 프레임워크 사용 (선택, Step 10에서)
